...
class JobQueueManager(QThread):
    ...
    core_rebalanced_signal = pyqtSignal()  # signal for core rebalancing

    def __init__(self, manager):
        super().__init__()
        self.manager = manager
        self.active_processes = []  # store refs to running processes
        self.should_stop = False
        self.last_core_check = time.time()  # track when last checked cores
    ...
    def run(self):
        ...
        while not self.should_stop:
            try:
                # check if any profiles need to be started based on current config
                self.start_active_profiles()
                
                # occasionally check if need to rebalance cores (10 seconds)
                ...
                if current_time - self.last_core_check > 10:
                    ...
                    # if over the core cap, trigger a rebalance
...
class MainUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.job_list.setSelectionMode(QListWidget.SingleSelection)  # items can be selected
        ...
        # use lambda to ensure proper connection
        self.toggle_status_btn.clicked.connect(self.toggle_job_status)  # remove lambda
        ...
        # init toggle_status_btn BEFORE loading profiles
